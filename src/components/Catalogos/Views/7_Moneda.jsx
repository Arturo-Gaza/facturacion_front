...
import Acciones from "../Actions/7_Moneda"; //###
//ICONOS
...
const Afectacion = (props) => {
  ...
  const [titulo, setTitulo] = useState("Moneda"); //### **
  ...
  if (!search0 && !search1) {
    ...
    if (search0 != "") {
      results = results.filter(
        (dato) =>
          dato.clave_moneda.toLowerCase().includes(search0.toLowerCase()) ||
          dato.descripcion_moneda.toLowerCase().includes(search0.toLowerCase())
      );
    }
  ...
  const descargarExcel = () => {
    props.props.setOpenLoadingScreen();
    requests
      .postToken(GET_MONEDA_EXCEL, search0) //### **
      .then((response) => {
        //props.props.setMessageSnackBar(response.data.message, 'success');
  ...
  if (ascNombre == true) {
    results = results.sort((a, b) =>
      a.clave_moneda.localeCompare(b.clave_moneda)
    ); //### **
  }

  if (descNombre == true) {
    results = results.sort((a, b) =>
      b.clave_moneda.localeCompare(a.clave_moneda)
    ); //### **
  }
              ...
              return (
                ...
                >
                  <StyledTableCell style={{ width: "1%", whiteSpace: "nowrap" }}>
                    <label className="textLabel4">{item.clave_moneda}</label>{" "}
                    {/* //### ** */}
                  </StyledTableCell>
