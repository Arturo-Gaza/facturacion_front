...
import Acciones from "../Actions/5_Roles"; //###
//ICONOS
...
const Afectacion = (props) => {
  ...
  const [titulo, setTitulo] = useState("Roles"); //### **
  ...
  const descargarExcel = () => {
    props.props.setOpenLoadingScreen();
    requests
      .postToken(GET_ROLES_EXCEL, search0) //### **
      .then((response) => {
        props.props.setMessageSnackBar(response.data.message, 'success');
  ...
  if (!search0 && !search1) {
    ...
    if (search0 != "") {
      results = results.filter(
        (dato) =>
          dato.nombre.toLowerCase().includes(search0.toLocaleLowerCase()) //### **
      );
    }
  ...
  if (descNombre == true) {
    results = results.sort((a, b) => b.nombre.localeCompare(a.nombre)); //### **
  }
