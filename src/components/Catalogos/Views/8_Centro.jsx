...
const Afectacion = (props) => {
    ...
    const [titulo, setTitulo] = useState("Centro")//### ** 
    ...
    if (!search0 && !search1) {
        ...
        if (search0 != "") {
            results = results.filter((dato) =>
                dato.clave_centro.toLowerCase().includes(search0.toLocaleLowerCase()) //### **
            );
        }
    ...
    if (ascNombre == true) {
        results = results.sort((a, b) => a.clave_centro.localeCompare(b.clave_centro)); //### **
    }

    if (descNombre == true) {
        results = results.sort((a, b) => b.clave_centro.localeCompare(a.clave_centro)); //### **
    }
                    ...
                    <TableBody>
                        {results.slice(inicio, fin).map((item) => {
                            return (
                                ...
                                >
                                    <StyledTableCell>
                                        <label className='textLabel4'>{item.clave_centro}</label> {/* //### ** */}
                                    </StyledTableCell>

                                    <StyledTableCell>
                                        <label className='textLabel4'>{item.descripcion_centro}</label> {/* //### ** */}
                                    </StyledTableCell>
