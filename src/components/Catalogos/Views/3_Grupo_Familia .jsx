...
import Acciones from "../Actions/3_Grupo_Familia "; //###
//ICONOS
...
const Afectacion = (props) => {
  ...
  const [titulo, setTitulo] = useState("Grupo de Artículos"); //### **
  ...
  const descargarExcel = () => {
    ...
    requests
      .postToken(GET_CAT_GRUPO_FAMILIA_EXCEL, search0) //### **
      .then((response) => {
  ...
  const GetListCatalogo = () => {
    props.props.setOpenLoadingScreen();
    requests
      .getToken(GET_LIST_CAT_GRUPO_FAMILIA) //### **
      .then((response) => {
        props.props.setMessageSnackBar(response.data.message, 'success');
  ...
  if (!search0 && !search1) {
    ...
    if (search0 != "") {
      results = results.filter(
        (dato) =>
          dato.clave_gpo_familia
            .toLowerCase()
            .includes(search0.toLocaleLowerCase()) ||
          dato.descripcion_gpo_familia
            .toLowerCase()
            .includes(search0.toLocaleLowerCase()) ||
          dato.descripcion_gpo_familia_2
            .toLowerCase()
            .includes(search0.toLocaleLowerCase()) //### **
      );
    }
  ...
  if (ascNombre == true) {
    results = results.sort((a, b) =>
      a.clave_gpo_familia.localeCompare(b.clave_gpo_familia)
    ); //### **
  }

  if (descNombre == true) {
    results = results.sort((a, b) =>
      b.clave_gpo_familia.localeCompare(a.clave_gpo_familia)
    ); //### **
  }
        ...
        >
          <TableHead>
            <TableRow>
              ...
              <StyledTableCell align="center">
                <label className="textLabel3">
                  Descripción Grupo de Artículos 2
                </label>
              </StyledTableCell>
            ...
            {results.slice(inicio, fin).map((item) => {
              return (
                ...
                >
                  ...
                  <StyledTableCell>
                    <label className="textLabel4">
                      {item.descripcion_gpo_familia_2}
                    </label>{" "}
                    {/* //### ** */}
                  </StyledTableCell>
