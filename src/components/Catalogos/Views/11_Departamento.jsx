...
const Afectacion = (props) => {
  ...
  const [titulo, setTitulo] = useState("Departamento"); //### **
  ...
  const GetListCatalogo = () => {
    ...
    Promise.all([
      requests.getToken(GET_LIST_DEPARTAMENTO),
      requests.getToken(GET_ALL_COMPRAS),
    ])
      .then(([responseDepto, responseCompras]) => {
        const usuariosCompras = responseCompras.data.data;

        const departamentosConResponsable = responseDepto.data.data.map(
          (departamento) => {
            const responsable = usuariosCompras.find(
              (usuario) =>
                usuario.id === departamento.id_usuario_responsable_compras
            );
            return {
              ...departamento,
              name: responsable ? responsable.name : "Sin responsable",
              apellidoP: responsable ? responsable.apellidoP : "",
              apellidoM: responsable ? responsable.apellidoM : "",
            };
          }
        );

        setListCatalogo({ departamentos: departamentosConResponsable });
  ...
  const descargarExcel = () => {
    props.props.setOpenLoadingScreen();
    requests
      .postToken(GET_DEPARTAMENTO_EXCEL, search0) //### **
      .then((response) => {
  ...
  var results = (_ListCatalogo.departamentos || []).sort((a, b) => a.id - b.id);
  ...
  return (
    <div style={{ padding: "1%" }}>
        ...
        >
          <TableHead>
            <TableRow>
              ...
              <StyledTableCell align="center">
                <label className="textLabel3">Responsable de compras</label>
              </StyledTableCell>
                ...
                >
                  ...
                  <StyledTableCell align="center">
                    <label className="textLabel4">
                      {`${item.name} ${item.apellidoP} ${item.apellidoM}`}
                    </label>
                  </StyledTableCell>
                  ...
                  <StyledTableCell align="center">
                    ...
                    <IconButton
                      ...
                      color="primary"
                      onClick={function () {
                        handleOpenTipo(item.id);
                        {
                          /* //### ** */
                        }
                      }}
                    >
                      <AssignmentIcon sx={{ color: "#0066CC" }} />
                    </IconButton>
                  </StyledTableCell>
                </StyledTableRow>
              );
      ...
      <Acciones
        ...
        id={id}
        titulo={titulo}
        handleOpenTipo={handleOpenTipo}
        modalTipoAbierto={modalTipoAbierto}
        handleCloseTipo={handleCloseTipo}
        setModalTipoAbierto={setModalTipoAbierto}
      />
    </div>
  );
};
