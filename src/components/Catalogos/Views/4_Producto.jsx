...
const Afectacion = (props) => {
    ...
    const descargarExcel = () => {
        ...
        requests
            .postToken(GET_CAT_PRODUCTOS_EXCEL, search0) //### **
            .then((response) => {
    ...
    const GetListCatalogo = () => {
        props.props.setOpenLoadingScreen();
        requests
            .getToken(GET_LIST_CAT_PRODUCTOS) //### **
            .then((response) => {
                props.props.setMessageSnackBar(response.data.message, "success");
    ...
    if (!search0 && !search1) {
        ...
        if (search0 != "") {
            results = results.filter((dato) => {
                const termino = search0.toLowerCase();

                return (
                    dato.clave_producto?.toLowerCase().includes(termino) ||
                    dato.descripcion_producto?.toLowerCase().includes(termino)
                );
            });
        }
    ...
    if (ascNombre == true) {
        results = results.sort((a, b) =>
            a.descripcion_producto.localeCompare(b.descripcion_producto)
        ); //### **
    }

    if (descNombre == true) {
        results = results.sort((a, b) =>
            b.descripcion_producto.localeCompare(a.descripcion_producto)
        ); //### **
    }
                    ...
                    <TableHead>
                        <TableRow>
                            <StyledTableCell align="center"
                                style={{ width: "fit-content", whiteSpace: "nowrap", padding: "0 8px" }}>
                                <label className="textLabel3">Clave Producto</label>
                            </StyledTableCell>

                            <StyledTableCell align="center">
                                <label className="textLabel3">Descripción Producto</label>
                            ...
                            <StyledTableCell align="center">
                                <label className="textLabel3">Unidad de Medida</label>
                            </StyledTableCell>

                            <StyledTableCell align="center">
                                <label className="textLabel3">Moneda</label>
                            </StyledTableCell>
                            ...
                            <StyledTableCell align="center">
                                <label className="textLabel3">Categoría</label>
                            </StyledTableCell>
