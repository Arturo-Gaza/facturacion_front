...
import { CREATE_CAT_ALMACEN, GET_CAT_ALMACEN_BY_ID, UPDATE_CAT_ALMACEN } from "../../../Constants/ApiConstants";
import { TextFieldGeneral } from '../../../Styles/TextField/TextField';
import { CAT_Almacen } from '../../../Utils/Validacion/Catolgos';
...
const Afectacion = (props) => {
    ...
    const Guardar = () => {
        ...
        requests
            .postToken(CREATE_CAT_ALMACEN, _ArrayCatalogo)//### **
            .then((response) => {
    ...
    const Actualizar = () => {
        props.props.props.setOpenLoadingScreen()
        requests
            .putToken(UPDATE_CAT_ALMACEN + props.id, _ArrayCatalogo)//### **
            .then((response) => {
                props.props.props.setMessageSnackBar(response.data.message, 'success');
                handleClose();
            }).catch((error) => {
    ...
    const GetArrayCatalogoByID = async () => {
        props.props.props.setOpenLoadingScreen()
        await requests
            .getToken(GET_CAT_ALMACEN_BY_ID + props.id) //### **
            .then((response) => {
    ...
    const handleSubmitValidado = (e) => {
        props.props.props.setOpenLoadingScreen();
        e.preventDefault();
        CAT_Almacen(_ArrayCatalogo).then((resp) => {//### **
            setErrorsCAT(resp);
            let boolValor = false;
                ...
                <Grid container className='marginComponets'>
                    ...
                    <Grid item xs={12} sm={12} md={12}>
                        <TextFieldGeneral
                            ...
                            name="clave_almacen"
                            // idprops={props.id}
                            value={_ArrayCatalogo.clave_almacen} /* //### ** */
                            error={errorsCAT.clave_almacen}
                ...
                <Grid container className='marginComponets'>
                    ...
                    <Grid item xs={12} sm={12} md={12}>
                        <TextFieldGeneral
                            ...
                            value={_ArrayCatalogo.descripcion_almacen} /* //### ** */
                            error={errorsCAT.descripcion_almacen}
                            onChange={handleArrayCatalogo}
                            size="small"
                            inputProps={{
                                maxLength: 200,
