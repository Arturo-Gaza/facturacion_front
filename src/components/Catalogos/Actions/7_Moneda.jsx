...
import { CREATE_MONEDA, GET_MONEDA_ID, UPDATE_MONEDA } from "../../../Constants/ApiConstants";
import { TextFieldGeneral } from '../../../Styles/TextField/TextField';
import { CAT_Moneda } from '../../../Utils/Validacion/Catolgos';
...
const Afectacion = (props) => {
    ...
    const Guardar = () => {
        ...
        requests
            .postToken(CREATE_MONEDA, _ArrayCatalogo)//### **
            .then((response) => {
    ...
    const Actualizar = () => {
        props.props.props.setOpenLoadingScreen()
        requests
            .putToken(UPDATE_MONEDA + props.id, _ArrayCatalogo)//### **
            .then((response) => {
                props.props.props.setMessageSnackBar(response.data.message, 'success');
                handleClose();
            }).catch((error) => {
    ...
    const GetArrayCatalogoByID = async () => {
        props.props.props.setOpenLoadingScreen()
        await requests
            .getToken(GET_MONEDA_ID + props.id) //### **
            .then((response) => {
    ...
    const handleSubmitValidado = (e) => {
        props.props.props.setOpenLoadingScreen();
        e.preventDefault();
        CAT_Moneda(_ArrayCatalogo).then((resp) => {//### **
            setErrorsCAT(resp);
            let boolValor = false;
                ...
                <Grid container className='marginComponets'>
                    ...
                    <Grid item xs={12} sm={12} md={12}>
                        <TextFieldGeneral
                            ...
                            name="clave_moneda"
                            // idprops={props.id}
                            value={_ArrayCatalogo.clave_moneda} /* //### ** */
                            error={errorsCAT.clave_moneda}
                ...
                <Grid container className='marginComponets'>
                    ...
                    <Grid item xs={12} sm={12} md={12}>
                        <TextFieldGeneral
                            ...
                            id="descripcion_moneda"
                            name="descripcion_moneda"
                            value={_ArrayCatalogo.descripcion_moneda} /* //### ** */
                            error={errorsCAT.descripcion_moneda}
                            onChange={handleArrayCatalogo}
                            size="small"
                            inputProps={{
                                maxLength: 200,
