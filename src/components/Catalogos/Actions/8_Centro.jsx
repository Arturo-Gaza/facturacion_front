...
import { CREATE_CENTRO, GET_CENTRO_ID, UPDATE_CENTRO } from "../../../Constants/ApiConstants";
import { TextFieldGeneral } from '../../../Styles/TextField/TextField';
import { CAT_Centro } from '../../../Utils/Validacion/Catolgos';
...
const Afectacion = (props) => {
    ...
    const Guardar = () => {
        ...
        requests
            .postToken(CREATE_CENTRO, _ArrayCatalogo)//### **
            .then((response) => {
    ...
    const Actualizar = () => {
        props.props.props.setOpenLoadingScreen()
        requests
            .putToken(UPDATE_CENTRO + props.id, _ArrayCatalogo)//### **
            .then((response) => {
                props.props.props.setMessageSnackBar(response.data.message, 'success');
                handleClose();
            }).catch((error) => {
    ...
    const GetArrayCatalogoByID = async () => {
        props.props.props.setOpenLoadingScreen()
        await requests
            .getToken(GET_CENTRO_ID + props.id) //### **
            .then((response) => {
    ...
    const handleSubmitValidado = (e) => {
        props.props.props.setOpenLoadingScreen();
        e.preventDefault();
        CAT_Centro(_ArrayCatalogo).then((resp) => {//### **
            setErrorsCAT(resp);
            let boolValor = false;
                ...
                <Grid container className='marginComponets'>
                    ...
                    <Grid item xs={12} sm={12} md={12}>
                        <TextFieldGeneral
                            ...
                            name="clave_centro"
                            // idprops={props.id}
                            value={_ArrayCatalogo.clave_centro} /* //### ** */
                            error={errorsCAT.clave_centro}
                ...
                <Grid container className='marginComponets'>
                    ...
                    <Grid item xs={12} sm={12} md={12}>
                        <TextFieldGeneral
                            ...
                            value={_ArrayCatalogo.descripcion_centro} /* //### ** */
                            error={errorsCAT.descripcion_centro}
                            onChange={handleArrayCatalogo}
                            size="small"
                            inputProps={{
                                maxLength: 200,
