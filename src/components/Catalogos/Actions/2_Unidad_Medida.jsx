...
import { CREATE_CAT_UNIDAD_MEDIDA, GET_CAT_UNIDAD_MEDIDA_BY_ID, UPDATE_CAT_UNIDAD_MEDIDA } from "../../../Constants/ApiConstants";
import { TextFieldGeneral } from '../../../Styles/TextField/TextField';
import { CAT_Unidad_Medida } from '../../../Utils/Validacion/Catolgos';
...
const Afectacion = (props) => {
    ...
    const Guardar = () => {
        ...
        requests
            .postToken(CREATE_CAT_UNIDAD_MEDIDA, _ArrayCatalogo)//### **
            .then((response) => {
    ...
    const Actualizar = () => {
        props.props.props.setOpenLoadingScreen()
        requests
            .putToken(UPDATE_CAT_UNIDAD_MEDIDA + props.id, _ArrayCatalogo)//### **
            .then((response) => {
                props.props.props.setMessageSnackBar(response.data.message, 'success');
                handleClose();
            }).catch((error) => {
    ...
    const GetArrayCatalogoByID = async () => {
        props.props.props.setOpenLoadingScreen()
        await requests
            .getToken(GET_CAT_UNIDAD_MEDIDA_BY_ID + props.id) //### **
            .then((response) => {
    ...
    const handleSubmitValidado = (e) => {
        props.props.props.setOpenLoadingScreen();
        e.preventDefault();
        CAT_Unidad_Medida(_ArrayCatalogo).then((resp) => {//### **
            setErrorsCAT(resp);
            let boolValor = false;
                ...
                <Grid container className='marginComponets'>
                    ...
                    <Grid item xs={12} sm={12} md={12}>
                        <TextFieldGeneral
                            ...
                            // idprops={props.id}
                            value={_ArrayCatalogo.clave_unidad_medida} /* //### ** */
                            error={errorsCAT.clave_unidad_medida}
                ...
                <Grid container className='marginComponets'>
                    ...
                    <Grid item xs={12} sm={12} md={12}>
                        <TextFieldGeneral
                            ...
                            value={_ArrayCatalogo.descripcion_unidad_medida} /* //### ** */
                            error={errorsCAT.descripcion_unidad_medida}
                            onChange={handleArrayCatalogo}
                            size="small"
                            inputProps={{
                                maxLength: 255,
