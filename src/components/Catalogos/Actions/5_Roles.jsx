...
import { CREATE_CAT_ROLES, GET_CAT_ROLES_BY_ID, UPDATE_CAT_ROLES } from "../../../Constants/ApiConstants";
import { TextFieldGeneral } from '../../../Styles/TextField/TextField';
import { CAT_Roles } from '../../../Utils/Validacion/Catolgos';
...
const Afectacion = (props) => {
    ...
    const Guardar = () => {
        ...
        requests
            .postToken(CREATE_CAT_ROLES, _ArrayCatalogo)//### **
            .then((response) => {
    ...
    const Actualizar = () => {
        props.props.props.setOpenLoadingScreen()
        requests
            .putToken(UPDATE_CAT_ROLES + props.id, _ArrayCatalogo)//### **
            .then((response) => {
                props.props.props.setMessageSnackBar(response.data.message, 'success');
                handleClose();
            }).catch((error) => {
    ...
    const GetArrayCatalogoByID = async () => {
        props.props.props.setOpenLoadingScreen()
        await requests
            .getToken(GET_CAT_ROLES_BY_ID + props.id) //### **
            .then((response) => {
    ...
    const handleSubmitValidado = (e) => {
        props.props.props.setOpenLoadingScreen();
        e.preventDefault();
        CAT_Roles(_ArrayCatalogo).then((resp) => {//### **
            setErrorsCAT(resp);
            let boolValor = false;
            ...
            <Box sx={{ padding: 5 }}>
                ...
                <Grid container className='marginComponets'>
                    <Grid item xs={12} sm={12} md={12} sx={{ paddingBottom: 1 }}>
                        <label className='textLabel2'>{"Nombre " + props.titulo}</label>
                    </Grid>
