...
import { CAT_Tipo } from "../../../Utils/Validacion/Catolgos";
...
const Afectacion = (props) => {
  ...
  const [_checkedMarcaModelo, setCheckedMarcaModelo] = useState(true);
  const [_ArrayCatalogo, setArrayCatalogo] = useState({
    descripcion: "",
    req_marca_modelo: true,
    habilitado: true,
  }); //### **
  ...
  const handleMarca = (e) => {
    ...
    setCheckedMarcaModelo(checked);
    setArrayCatalogo((prev) => ({
      ...prev,
      req_marca_modelo: checked,
    }));
  };
  ...
  const Guardar = () => {
    ...
    requests
      .postToken(CREATE_TIPO, _ArrayCatalogo) //### **
      .then((response) => {
  ...
  const Actualizar = () => {
    props.props.props.setOpenLoadingScreen();
    requests
      .putToken(UPDATE_TIPO + props.id, _ArrayCatalogo) //### **
      .then((response) => {
        props.props.props.setMessageSnackBar(response.data.message, "success");
        handleClose();
      }).catch((error) => {
  ...
  const GetArrayCatalogoByID = async () => {
    props.props.props.setOpenLoadingScreen();
    await requests
      .getToken(GET_TIPO_ID + props.id) //### **
      .then((response) => {
        setArrayCatalogo(response.data.data);
        setChecked(response.data.data.habilitado);
        setCheckedMarcaModelo(response.data.data.req_marca_modelo);
      }).catch((error) => {
        error.response.data.errors.forEach(element => {  //forEach Ya quedo
          props.props.props.setMessageSnackBar(element, 'warning');
        });
  ...
  const handleSubmitValidado = (e) => {
    props.props.props.setOpenLoadingScreen();
    e.preventDefault();
    CAT_Tipo(_ArrayCatalogo).then((resp) => {
      //### **
      setErrorsCAT(resp);
      let boolValor = false;
  ...
  return (
    ...
    >
      <Box sx={{ padding: 5 }}>
        ...
        <Grid container className="marginComponets">
          <Grid item xs={12} sm={12} md={12}>
            <label className="textLabel2">Requiere Marca</label>
            <Checkbox
              className="ckBox1"
              id="req_marca_modelo"
              name="req_marca_modelo"
              checked={_checkedMarcaModelo}
